...
// signup.js (Updated without alert, using popup modal)
...
// }
  ...
  if (!/^[6-9][0-9]{9}$/.test(mobileNoVal)) {
    showMessageModal("❌ Invalid mobile number. Must start with 6-9 and be 10 digits.");
    return;
  }
  ...
  fetch("http://localhost:8080/studentsignup", {
  method: "POST",
...
.then(async response => {
  ...
  // Adjust this condition based on your backend's convention:
