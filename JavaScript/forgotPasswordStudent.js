...
emailForm.addEventListener("submit", async function(e){
  ...
  try {
    const res = await fetch("http://localhost:8080/otpGenerationForForgotPasswordStudents", {
      method: "POST",
...
document.getElementById("verifyOtpBtn").addEventListener("click", async function() {
  ...
  try {
    const url = `http://localhost:8080/verifyOtpStudents/${otp}/?email=${encodeURIComponent(verifiedEmail)}`;
